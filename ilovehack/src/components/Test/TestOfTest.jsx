...
class Test extends Component {
    ...
    toggleChecked = () => {
        const checkedOrNot = this.state.questions[0].checked
        console.log(checkedOrNot, 'el checked')
        this.setState({ checkedOrNot: !checkedOrNot })
        console.log(checkedOrNot, 'el checked 2')
    }

    handleChange = event => {
        const { value } = event.target;
        const checkedOrNot = this.state.questions[0].checked
        this.setState({ answers: [...this.state.answers, value], checkedOrNot: !checkedOrNot });
        };
            ...
            <p>{questions[number].question}</p>
                { questions[number].answers ? questions[number].answers.map((answer, index) => {
                    return (
                        ...
                        <input onChange={e => this.handleChange(e)} onFocus={() => this.toggleChecked()} id={answer} type={questions[number].inputType} name={number} value={answer} checked={questions[number].checked}/>
                        </div>
                        )
